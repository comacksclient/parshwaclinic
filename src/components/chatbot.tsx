...
import { MessageCircle, Send, X, User, ChevronRight, Sparkles } from "lucide-react";
    ...
    >
        <div className="flex max-w-[85%] gap-3 flex-row items-end">
            ...
            <div className="bg-white p-4 rounded-[24px] rounded-bl-sm shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-100 flex gap-1.5 items-center">
                ...
                <span className="w-2 h-2 bg-[#1A1A1A]/60 rounded-full animate-bounce delay-100"></span>
                <span className="w-2 h-2 bg-[#1A1A1A]/80 rounded-full animate-bounce delay-200"></span>
            </div>
        </div>
    </motion.div>
);
...
const DoctorCarousel = ({ doctors, onSelectDoctor }: { doctors: Doctor[], onSelectDoctor: (id: string, name: string) => void }) => {
    ...
    return (
        <div className="relative mt-2">
            <div
                className="relative overflow-hidden rounded-[24px] border border-gray-100 shadow-sm bg-white"
            ...
            >
                ...
                <div className="p-5">
                    <h3 className="font-black text-[#1A1A1A] text-lg tracking-tight mb-1">{currentDoctor.name}</h3>
                    <p className="text-[10px] text-[#AEE9F5] bg-[#1A1A1A] inline-block px-2 py-1 rounded-md font-black uppercase tracking-widest mb-4">
                        {currentDoctor.specialty}
                    </p>

                    <button
                        onClick={() => onSelectDoctor(`ACTION_DETAILS_${currentDoctor.id}`, `Details: ${currentDoctor.name}`)}
                        className="w-full bg-[#FAFAFC] border border-gray-200 hover:border-[#1A1A1A] text-[#1A1A1A] text-xs font-bold py-3 px-4 rounded-[16px] transition-all duration-300"
                    >
                        View Profile
                    </button>
                </div>
            ...
            {doctors.length > 1 && (
                <button
                    onClick={handleNext}
                    className="absolute right-2 top-[40%] bg-white/90 backdrop-blur-sm hover:bg-[#1A1A1A] hover:text-white text-[#1A1A1A] p-2 rounded-full shadow-lg transition-all"
                >
                ...
                <div className="flex justify-center gap-1.5 mt-4">
                    {doctors.map((_, idx) => (
                        <button
                            ...
                            className={`h-1.5 rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-6 bg-[#1A1A1A]' : 'w-2 bg-gray-200'}`}
                        />
                    ))}
                </div>
            )}
...
export function Chatbot() {
        ...
        return (
            <div className="flex flex-col gap-3 w-full">
                {msg.image && !msg.doctors && (
                    <div className="relative w-full h-36 rounded-[16px] overflow-hidden mb-2 shadow-sm">
                        <img src={msg.image} alt="Visual" className="w-full h-full object-cover" />
                    </div>
                )}

                <div className="whitespace-pre-wrap font-medium">{textContent}</div>
                ...
                {msg.type === "booking_form" && msg.fields && (
                    <form onSubmit={(e) => handleBookingSubmit(e, msg.fields!)} className="flex flex-col gap-4 mt-3 bg-[#FAFAFC] p-4 rounded-[20px] border border-gray-100">
                        {msg.fields.map((field) => (
                            <div key={field.id} className="flex flex-col gap-1.5">
                                <label htmlFor={field.id} className="text-[10px] font-black uppercase tracking-widest text-gray-400 pl-1">
                                    {field.label} {field.required && <span className="text-[#AEE9F5]">*</span>}
                                </label>
                                <input
                                    id={field.id}
                                    type={field.type}
                                    ...
                                    className="px-4 py-3 text-sm border border-gray-200 rounded-[16px] focus:outline-none focus:border-[#1A1A1A] focus:ring-4 focus:ring-gray-100 transition-all font-bold text-[#1A1A1A]"
                                />
                            </div>
                        ))}
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full bg-[#1A1A1A] hover:bg-[#AEE9F5] hover:text-[#1A1A1A] text-white font-black py-3 px-4 rounded-[16px] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed mt-2 shadow-[0_10px_20px_rgba(26,26,26,0.1)]"
                        >
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                        {msg.buttons.map((btn, idx) => (
                            <button
                                ...
                                className="text-xs font-bold border border-[#1A1A1A]/20 text-[#1A1A1A] bg-white px-4 py-2 rounded-full hover:bg-[#1A1A1A] hover:text-white transition-all duration-300 active:scale-95 shadow-sm"
                            >
                                {btn.label}
                            </button>
                        ))}
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-4 font-sans">
                    ...
                    >
                        ...
                        <div className="bg-[#1A1A1A]/95 backdrop-blur-md p-5 flex items-center justify-between shrink-0 relative overflow-hidden border-b border-white/10">
                            ...
                            <button
                                ...
                                className="text-white/60 hover:text-white hover:bg-white/10 transition-all p-2 rounded-full relative z-10"
                            >
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                                ...
                                >
                                    <div className={`flex max-w-[85%] gap-3 items-end ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                        ...
                                        <div className="flex flex-col gap-1">
                                            ...
                                            <span className={`text-[10px] font-black uppercase tracking-widest text-gray-300 ${msg.role === 'user' ? 'text-right' : 'text-left ml-2'}`}>
                        ...
                        <div className="bg-white/50 backdrop-blur-md border-t border-white/20 shrink-0 p-4 rounded-b-[40px]">
                            ...
                            <div className="mt-3 text-center">
                                <a href="https://comacks.com" target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold uppercase tracking-widest text-gray-300 hover:text-gray-500 transition-colors">
                                    Powered by <span className="text-gray-400">Comacks</span>
                                </a>
                            </div>
                        </div>
                    </motion.div>
            ...
            {/* Toggle & WhatsApp Buttons */}
            <div className={`transition-all duration-500 ease-[0.22,1,0.36,1] ${isOpen ? 'translate-y-4 opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}`}>
                <a
            ...
            >
                ...
                {!isOpen && (
                    <span className="absolute top-0 right-0 w-4 h-4 bg-[#AEE9F5] border-[3px] border-[#1A1A1A] rounded-full animate-pulse group-hover:border-[#AEE9F5] transition-colors"></span>
                )}
            </button>
        </div>
    );
