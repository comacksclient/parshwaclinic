...
You are classifying user intent for Parshwa Dental Clinic.
...
async function generalChatNode(state: typeof AgentState.State) {
    ...
    const prompt = `
You are the friendly AI receptionist for Parshwa Dental Clinic in Sabarmati, Ahmedabad.
...
- Doctors:
  - Dr. Shrenik Shah – Dental Surgeon.
  - Dr. Dimple Shah – Dental Surgeon.
- Location: 21, 1st Floor, Trishla Complex Opp. Podar School New CG Road Chandkheda, Ahmedabad – 382424.
- Hours: Monday–Saturday, 10:00 AM–2 PM and 5–9 PM (Closed Sundays).
- Contact: +91 93288 20346, [shrenik_shah16@yahoo.com](mailto:shrenik_shah16@yahoo.com)
...
8. For location queries, include this exact sentence in your own words: We are located at: 21, 1st Floor, Trishla Complex Opp. Podar School New CG Road Chandkheda, Ahmedabad – 382424. You can view us on Maps here: [https://www.google.com/maps?q=Parshwa+Dental+Clinic+Chandkheda+Ahmedabad](https://www.google.com/maps?q=Parshwa+Dental+Clinic+Chandkheda+Ahmedabad)
...
function retrieveDoctorProfileNode(state: typeof AgentState.State) {
    const ridam = DOCTORS_DB.dr_shrenik;
    const tanmay = DOCTORS_DB.dr_dimple;
    const responseText = `👨‍⚕️ Meet Our Specialists at Parshwa Dental Clinic`;
...
function retrieveDoctorDetailsNode(state: typeof AgentState.State) {
    const docId = state.selected_doctor_id || "dr_shrenik";
    // @ts-ignore
    const doc = DOCTORS_DB[docId] || DOCTORS_DB.dr_shrenik;
    const responseText = `${doc.name}\n${doc.specialty}\n\n${doc.short_bio}\n\nAvailability: Mon-Sat (10:00 AM - 2 PM & 5 PM - 9 PM)`;
...
function emergencyNode(state: typeof AgentState.State) {
    const responseText = "🚨 We are here for you.\n\nIf you are in pain, please visit us in Sabarmati immediately or call us.\n\n📞 +91 93288 20346";
    ...
    return {
        ...
        final_response: {
            ...
            text: responseText,
            buttons: [
                { label: "Call Now", payload: "tel:+919328820346" },
...
export async function POST(req: NextRequest) {
    try {
        ...
        return NextResponse.json(
            {
                type: "text",
                text: "I'm having trouble connecting. Please call us at +91 93288 20346 for assistance.",
            },
            { status: 500 }
        );
    }
